...
class FaceWindow extends KAGPlugin {
	...
	// コンストラクタ
	function FaceWindow(w, elm, restoreflg=false)
	{
		// 初期化。ここでやらないと onRestore() で使うので
		this.w           = w;
		ct_org           = w.tagHandlers.ct 	if (!restoreflg);
		cm_org           = w.tagHandlers.cm 	if (!restoreflg);
		w.tagHandlers.ct = ct_new		if (!restoreflg);
		w.tagHandlers.cm = cm_new		if (!restoreflg);
...
//			p.srcheight = e.srcheight if (e.srcheight!== void);
	...
	{
		var dic = f.FaceWindow;
		finalize(true);		// ct_org/cm_orgは元に戻さない
		if (dic === void)
			return;
		FaceWindow(w, dic, true); // 初期化(ct_org/cm_orgは設定しない)
