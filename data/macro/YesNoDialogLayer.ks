...
class ImageDialogLayerPlugin extends KAGPlugin {
	...
	var options = %[
		...
		image    : "image/dialog/YesNoDialog", 	// YesNoダイアログの画像
		...
		yesbutton : %[			// 「はい」ボタン
			...
			image  : "image/dialog/YesNoDialog_YesButton",	// 画像
			left   : 125,		// 表示位置
			top    : 300
		],
		nobutton : %[			// 「いいえ」ボタン
			button : void,		// ボタンインスタンス
			image  : "image/dialog/YesNoDialog_NoButton",// 画像
			left   : 475,		// 表示位置
			top    : 300
		],
		okbutton : %[			// 「はい」ボタン
			button : void,		// ボタンインスタンス
			image  : "image/dialog/OKDialog_OKButton",	// 画像
			left   : 300,		// 表示位置
			top    : 300
		]
	];
...
//	var maincond_interrupted, extracond_interrupted;
	...
	function ImageDialogLayerPlugin(window, elm)
	{
		...
		movecursorx = win.scWidth/2+175;
		movecursory = win.scHeight/2+150;
...
// ここ以下はwin(=kag)の関数置き換え用
	...
	function onCloseQuery()	// incontextof win
	{
		...
		// 質問ダイアログを表示してすぐ終わる
		obj.askYesNoLayer("ゲームを終了しますか？", onCloseQuery_2nd);
